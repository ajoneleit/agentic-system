...
from .claude_cli_mcp_client import ClaudeCodeMCPClient
...
class EnhancedMCPClient(ClaudeCodeMCPClient):
    ...
    async def create_message(
        ...
        **kwargs,
        ...
        try:
            ...
            workspace_path = await self._setup_workspace_with_events(
                task_id, kwargs.get("workspace_path")
            )
                ...
                MCPEvent(
                    ...
                    data={
                        ...
                        "commands_executed": len(
                            result.get("metadata", {}).get("commands_executed", [])
                        ),
            ...
            await emit_mcp_event(
                MCPEvent(
                    ...
                    task_id=UUID(task_id) if task_id else None,
                    session_id=session_id,
                    data={"error_type": e.__class__.__name__, "error_message": str(e)},
                )
            )
            raise
    ...
    async def _setup_workspace_with_events(
        self, task_id: str, base_path: Optional[str] = None
    ) -> str:
        ...
        await emit_mcp_event(
            MCPEvent(
                ...
                source="enhanced_mcp_client",
                task_id=UUID(task_id) if task_id else None,
                data={"task_id": task_id, "base_path": base_path, "status": "creating"},
            )
        )
        ...
        try:
            ...
            await emit_mcp_event(
                MCPEvent(
                    ...
                    source="enhanced_mcp_client",
                    task_id=UUID(task_id) if task_id else None,
                    data={"task_id": task_id, "error": str(e), "status": "failed"},
                )
            )
            raise
        ...
        try:
            ...
            await emit_mcp_event(
                MCPEvent(
                    ...
                    task_id=task_id,
                    duration_ms=(time.time() - start_time) * 1000,
                    data={"args": args, "timeout_reason": "operation_timeout"},
                )
            )
            raise
    ...
    async def start_server_with_events(
        ...
        try:
            ...
            await emit_mcp_event(
                MCPEvent(
                    ...
                    source="enhanced_mcp_client",
                    server_name=server_name,
                    data={"connection_type": "mcp", "protocol_version": "1.0"},
                )
            )
    ...
    async def track_resource_operation(
        ...
        await emit_mcp_event(
            MCPEvent(
                ...
                server_name=server_name,
                agent_id=agent_id,
                data={"resource_path": resource_path, "operation": operation_type},
            )
        )
        ...
        for task_id, session_id in self.active_sessions.items():
            await emit_mcp_event(
                MCPEvent(
                    ...
                    task_id=UUID(task_id) if task_id else None,
                    session_id=session_id,
                    data={"shutdown_reason": "client_close", "forced_termination": True},
                )
            )
