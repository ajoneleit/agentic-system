...
def _create_error_dict(error_type: str, message: str, fatal: bool = False) -> dict[str, Any]:
    ...
    """
    return {"type": error_type, "message": message, "fatal": fatal}
...
class ClaudeCodeClient:
        ...
        try:
                    ...
                    if file_path.exists() and file_path.is_file():
                        try:
                            ...
                            if file_size > 1_000_000:  # Skip files larger than 1MB
                                logger.warning(
                                    f"Skipping large file in context: {file_path} ({file_size} bytes)"
                                )
                                continue
            ...
            if use_temp_file:
                ...
                fd, temp_file_path = tempfile.mkstemp(
                    suffix=".md", prefix="claude_prompt_", text=True
                )
            ...
            if not response.strip():
                ...
                raise APIResponseError(
                    status_code=200, response_body="", reason="Claude Code returned empty response"
                )
            ...
            try:
                ...
                # Check for required fields based on actual Claude CLI response format
                if "type" not in json_response:
                    raise APIResponseError(
                        status_code=200,
                        response_body=response,
                        reason="Missing required field in JSON response: type",
                    )

                # Check for fatal errors based on actual Claude CLI response format
                if json_response.get("is_error", False):
                    error_result = json_response.get("result", "Unknown error")
                    raise APIResponseError(
                        status_code=200,
                        response_body=response,
                        reason=f"Claude Code error: {error_result}",
                    )
                ...
                logger.info(
                    ...
                    is_error=json_response.get("is_error", False),
            ...
            logger.error("Failed to execute Claude Code", error=str(e), exc_info=True)
    ...
    async def create_message_for_code(
        self, messages: list[dict[str, str]], task_type: str = "code", **kwargs
    ) -> dict[str, Any]:
                ...
                try:
                    ...
                    if process.returncode == 0:
                        ...
                        if any(
                            ...
                            for keyword in ["claude", "version", "usage", "commands", "help"]
                        ):
                            logger.info(
                                f"Claude Code available (checked with {' '.join([self.cli_command] + args)})"
                            )
                            return True
                except Exception:
                    continue
    ...
    def convert_text_to_json_response(
        ...
        if workspace_dir and workspace_dir.exists():
            try:
                ...
                errors.append(
                    _create_error_dict(
                        "Unknown", f"Failed to scan workspace directory: {str(e)}", fatal=False
                    )
                )
        ...
        return _create_json_response(
            status=status, summary=summary, files=files, errors=errors, metrics=metrics
        )
