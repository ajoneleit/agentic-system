...
class HealthCheckResult:
    ...
    def __init__(
        self, component: str, status: str, message: str, details: Optional[dict[str, Any]] = None
    ):
...
class SystemHealthChecker:
    ...
    async def _check_python_version(self) -> None:
        ...
        try:
            ...
            self.results.append(
                HealthCheckResult("python_version", "ERROR", f"Failed to check Python version: {e}")
            )
    ...
    async def _check_required_directories(self) -> None:
        ...
        if missing_dirs:
            ...
            self.results.append(
                HealthCheckResult("directories", "OK", "All required directories present")
            )
    ...
    async def _check_configuration(self) -> None:
        ...
        try:
            ...
            critical_checks = [
                ...
                (
                    settings.verification.minimum_coverage >= 0,
                    "minimum_coverage must be non-negative",
                ),
            ...
            self.results.append(
                HealthCheckResult("configuration", "ERROR", f"Failed to load configuration: {e}")
            )
    ...
    async def _check_logging(self) -> None:
        ...
        try:
            ...
            self.results.append(HealthCheckResult("logging", "OK", "Logging system initialized"))

        except Exception as e:
            self.results.append(
                HealthCheckResult("logging", "ERROR", f"Logging initialization failed: {e}")
            )
    ...
    async def _check_api_configuration(self) -> None:
        ...
        try:
            ...
            self.results.append(
                HealthCheckResult(
                    "api_config", "ERROR", "Invalid API key configuration", {"error": str(e)}
                )
            )
        except Exception as e:
            self.results.append(
                HealthCheckResult("api_config", "ERROR", f"API configuration check failed: {e}")
            )
    ...
    async def _check_claude_connectivity(self) -> None:
        ...
        try:
            ...
            if not settings.api.key:
                self.results.append(
                    HealthCheckResult("claude_api", "SKIPPED", "No API key configured")
                )
                return
            ...
            async with ClaudeClient() as client:
                ...
                if response and response.content:
                    ...
                    self.results.append(
                        HealthCheckResult("claude_api", "ERROR", "Unexpected API response format")
                    )
            ...
            self.results.append(
                HealthCheckResult(
                    "claude_api", "ERROR", f"API error: {e}", {"error_code": e.error_code}
                )
            )
        except Exception as e:
            self.results.append(
                HealthCheckResult("claude_api", "ERROR", f"Failed to connect to Claude API: {e}")
            )
    ...
    async def _check_interfaces(self) -> None:
        ...
        try:
            ...
            self.results.append(
                HealthCheckResult("interfaces", "ERROR", f"Interface instantiation failed: {e}")
            )
        ...
        if missing_deps:
            self.results.append(
                HealthCheckResult(
                    ...
                    {"missing": missing_deps, "hint": f"Run: pip install {' '.join(missing_deps)}"},
                )
            )
        else:
            self.results.append(
                HealthCheckResult("dependencies", "OK", "All required dependencies installed")
            )
