...
class FileManager:
    ...
    def __init__(
        ...
        logger.info(
            "FileManager initialized", base_path=str(self.base_path), max_file_size=max_file_size
        )

    async def read_file(self, relative_path: Union[str, Path], encoding: str = "utf-8") -> str:
        """Safely read a file.
        ...
        try:
            ...
            logger.debug("File read successfully", path=str(relative_path), size=len(content))

            return content

        except Exception as e:
            logger.error("Failed to read file", path=str(relative_path), error=str(e))
    ...
    async def write_file(
        ...
        async with self._get_file_lock(file_path):
            ...
            try:
                ...
                logger.info(
                    "File written successfully", path=str(relative_path), size=len(content_bytes)
                )
                ...
                logger.error("Failed to write file", path=str(relative_path), error=str(e))
    ...
    async def delete_file(
        self, relative_path: Union[str, Path], backup_before_delete: bool = True
    ) -> None:
        ...
        async with self._get_file_lock(file_path):
            ...
            try:
                ...
                logger.info("File deleted", path=str(relative_path))

            except Exception as e:
                logger.error("Failed to delete file", path=str(relative_path), error=str(e))
                raise FileManagerError(f"Failed to delete file: {e}")

    async def copy_file(
        self, source_path: Union[str, Path], dest_path: Union[str, Path], overwrite: bool = False
    ) -> Path:
        """Copy a file safely.
        ...
        async with self._get_file_lock(source), self._get_file_lock(dest):
            ...
            try:
                ...
                logger.info("File copied", source=str(source_path), dest=str(dest_path))
    ...
    async def move_file(
        ...
        async with self._get_file_lock(source), self._get_file_lock(dest):
            ...
            try:
                ...
                logger.info("File moved", source=str(source_path), dest=str(dest_path))
    ...
    async def list_files(
        ...
        try:
            ...
            logger.error("Failed to list files", path=str(relative_path), error=str(e))
            raise FileManagerError(f"Failed to list files: {e}")

    async def get_file_info(self, relative_path: Union[str, Path]) -> dict[str, Any]:
    ...
    async def create_directory(
        self, relative_path: Union[str, Path], parents: bool = True, exist_ok: bool = True
    ) -> Path:
        """Create a directory.
        ...
        try:
            ...
            logger.info("Directory created", path=str(relative_path))

            return dir_path

        except Exception as e:
            logger.error("Failed to create directory", path=str(relative_path), error=str(e))
    ...
    async def cleanup_old_files(
        ...
        for file_path in files:
            ...
            try:
                ...
                logger.warning(
                    "Failed to process file for cleanup", path=str(file_path), error=str(e)
                )

        logger.info("Cleanup completed", files_deleted=len(deleted_files), dry_run=dry_run)
