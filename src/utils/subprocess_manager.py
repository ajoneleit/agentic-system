...
from src.core.exceptions import SubprocessError, SubprocessTimeoutError, SubprocessMemoryError
    ...
    async def check_limits(self, current_output_size: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if current_output_size > self.output_limit_bytes:
                return True, f"Output limit exceeded: {current_output_size} > {self.output_limit_bytes} bytes"
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *cmd,
                ...
                cwd=cwd,
                env=env,
                limit=max_output_mb * 1024 * 1024 if not stream_output else 1024 * 1024  # 1MB buffer for streaming
            )
            ...
            monitor = ProcessMonitor(
                process.pid,
                max_memory_mb,
                max_output_mb * 1024 * 1024
            )
            
            # Create monitoring task
            monitor_task = asyncio.create_task(
                self._monitor_process(process, monitor, timeout)
            )
