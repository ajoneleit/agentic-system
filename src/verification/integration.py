...
class TaskVerificationResult:
    ...
    def __post_init__(self):
        ...
        for artifact_id, results in self.artifact_results.items():
...
class VerificationIntegration:
    ...
    async def generate_repair_tasks(
        ...
        if syntax_errors:
            task = await self._create_syntax_repair_task(
                verification_result.task_id,
                syntax_errors
            )
        ...
        if test_failures:
            task = await self._create_test_repair_task(
                verification_result.task_id,
                test_failures
            )
            repair_tasks.append(task)
            ...
            if artifact.id in verification_results:
                for result in verification_results[artifact.id]:
                    if not result.success:
                        ...
                        suggestion = await self.repair_analyzer.analyze_failure(
                            artifact, result
                        )
                        if suggestion:
    ...
    async def _create_syntax_repair_task(
        ...
        syntax_errors: List[Tuple[UUID, VerificationResult]]
    ) -> Task:
...
Use the error messages and suggestions to correct the syntax issues."""
    ...
    async def _create_compilation_repair_task(
        self,
        original_task_id: UUID,
        compilation_errors: List[Tuple[UUID, VerificationResult]]
    ) -> Task:
...
Ensure the code compiles successfully without errors."""
    ...
    async def _create_test_repair_task(
        self,
        original_task_id: UUID,
        test_failures: List[Tuple[UUID, VerificationResult]]
    ) -> Task:
