...
class QualityGateSystem:
    ...
    def __init__(self, min_quality_score: float = 0.8):
        ...
        logger.info("Quality gate system initialized", min_quality_score=min_quality_score)

    async def validate_cli_output(
        self, output: str, task: dict[str, Any], requirements: Optional[dict[str, Any]] = None
    ) -> QualityResult:
        ...
        passed = (
            quality_score >= requirements.get("min_code_quality", 0.8)
            and security_score >= requirements.get("min_security_score", 0.9)
            and coverage_score >= requirements.get("min_test_coverage", 0.7)
            and complexity_score >= requirements.get("max_complexity", 0.6)
        )
        ...
        return QualityResult(
            ...
            issues=issues,
            timestamp=datetime.utcnow(),
            metadata={"task": task, "requirements": requirements, "code_blocks": len(code_blocks)},
        )
    ...
    async def generate_feedback_prompt(
        self, quality_result: QualityResult, original_task: dict[str, Any]
    ) -> str:
