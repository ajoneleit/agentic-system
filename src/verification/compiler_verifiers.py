...
class JavaScriptVerifier(BaseVerifier):
    ...
    async def _verify_with_node(self, code: str, filename: Path, result: VerificationResult) -> None:
    ...
    async def verify(self, artifact: Artifact, context: Optional[Dict[str, Any]] = None) -> VerificationResult:
        ...
        try:
            ...
            if not await self._check_tsc_available():
                result.add_warning("TypeScript compiler not available, falling back to JavaScript verification")
...
def get_verifier_for_language(language: str, config: Optional[VerificationConfig] = None) -> Optional[BaseVerifier]:
