...
from .test_verifiers import JavaScriptTestVerifier, PythonTestVerifier
...
class PipelineResult:
    ...
    def complete(self) -> None:
        ...
        for artifact_id, _results in self.verification_results.items():
...
class VerificationPipeline:
    ...
    def _initialize_verifiers(self) -> dict[str, list[BaseVerifier]]:
        ...
        verifiers["typescript"].append(
            JavaScriptTestVerifier(self.config)
        )  # JS test runner works for TS

        return verifiers

    async def verify_artifacts(
        self, artifacts: list[Artifact], verification_config: Optional[VerificationConfig] = None
    ) -> PipelineResult:
    ...
    def _group_artifacts(self, artifacts: list[Artifact]) -> list[tuple[str, list[Artifact]]]:
        ...
        other_artifacts = [
            a for a in artifacts if a.type not in [ArtifactType.SOURCE_CODE, ArtifactType.TEST_CODE]
        ]
        if other_artifacts:
    ...
    async def _verify_group(
        self, artifacts: list[Artifact], config: VerificationConfig, result: PipelineResult
    ) -> None:
    ...
    async def _verify_single_artifact(
        self, artifact: Artifact, config: VerificationConfig, result: PipelineResult
    ) -> None:
        ...
        for verifier in verifiers:
            ...
            if artifact.type == ArtifactType.SOURCE_CODE:
                if isinstance(verifier.verification_type, VerificationType):
                    if (
                        verifier.verification_type == VerificationType.SYNTAX
                        and config.enable_syntax_check
                    ):
                        should_run = True
                    elif (
                        verifier.verification_type == VerificationType.COMPILATION
                        and config.enable_compilation
                    ):
                        should_run = True
            elif artifact.type == ArtifactType.TEST_CODE:
    ...
    async def _get_verification_context(
        self, artifact: Artifact, verifier: BaseVerifier
    ) -> dict[str, Any]:
    ...
    def _generate_pipeline_suggestions(self, result: PipelineResult) -> None:
        ...
        if result.compilation_checks > 0 and result.compilation_passed < result.compilation_checks:
            result.suggestions.append("Ensure code compiles successfully before running tests")
...
async def verify_task_artifacts(
    task: Task, artifact_manager, config: Optional[VerificationConfig] = None
