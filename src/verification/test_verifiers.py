...
class PythonTestVerifier(BaseVerifier):
    ...
    async def _prepare_test_environment(
        self, artifact: Artifact, context: Optional[dict[str, Any]]
    ) -> Path:
            ...
            if context and "source_artifacts" in context:
                for source_artifact in context["source_artifacts"]:
                    if isinstance(source_artifact, Artifact):
                        ...
                        if (
                            not source_content
                            and source_artifact.path
                            and source_artifact.path.exists()
                        ):
    ...
    async def _run_pytest(self, test_dir: Path) -> TestSuiteResult:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                *cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=str(test_dir)
            )
    ...
    def _process_test_results(
        self, test_suite_result: TestSuiteResult, result: VerificationResult
    ) -> None:
        ...
        if test_suite_result.failed_tests > 0 or test_suite_result.error_tests > 0:
            ...
            for test_result in test_suite_result.test_results:
                if test_result.status in ["failed", "error"]:
                    result.add_error(
                        f"Test '{test_result.name}' failed: {test_result.error_message}"
                    )
        else:
...
class JavaScriptTestVerifier(BaseVerifier):
    ...
    async def _detect_test_runner(self) -> Optional[str]:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                "npx", "jest", "--version", stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            await proc.communicate()
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                "npx", "mocha", "--version", stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            await proc.communicate()
            if proc.returncode == 0:
                return "mocha"
    ...
    async def _run_mocha(self, test_dir: Path) -> TestSuiteResult:
        ...
        cmd = [
            ...
            "json",
            "--timeout",
            str(self.config.test_timeout_seconds * 1000),
        ]
