...
from src.core.interfaces import Task, TaskStatus, TaskContext, Agent
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    logger.info(
                        f"Retrying task in {self.retry_delay} seconds",
                        task_id=task_id
                    )
    ...
    async def _generate_final_report(
        ...
        report = {
            ...
            "completed_tasks": [
                ...
                for task in all_tasks if task.id in completed_tasks
            ],
            "failed_tasks": [
                {
                    "id": str(task.id),
                ...
                for task in all_tasks if task.id in failed_tasks
            ],
        ...
        if self.failure_reports:
            ...
            if json_errors > 0:
                report["recommendations"].append(
                    "Claude CLI responses not in expected JSON format"
                )
