...
from src.core.failure_analyzer import FailureAnalyzer, RetryStrategy
...
class MetaAgent(MetaAgentInterface):
    ...
    def __init__(self, artifact_storage_path: Optional[Path] = None):
        ...
        self.coordinator = AgentCoordinator(self.task_manager, self.communication_hub)
        ...
        self._project_path: Path = (
            self.artifact_storage_path / "default_project"
        )  # Default project path
        self._workspace_path: Path = self._project_path / "workspace"  # Default workspace path

        logger.info(
            "MetaAgent initialized with retry support",
    ...
    async def process_request(self, user_request: str) -> Result[ProjectResult]:
        ...
        try:
            ...
            context = TaskContext(
                ...
                shared_memory={
                    "project_summary": (
                        self._active_project.get("project_summary", "")
            ...
            await self._create_project_manifest(user_request, task_results)
    ...
    async def decompose_task(self, user_prompt: str) -> list[Task]:
        ...
        try:
            ...
            for task_def in analysis["tasks"]:
                ...
                complexity = task_def.get("estimated_complexity") or task_def.get(
                    "complexity", "moderate"
                )
            ...
            for task_def, task in zip(analysis["tasks"], tasks):
                ...
                for dep_name in dep_list:
                    ...
                    if dep_name in task_map:
                        ...
                        logger.debug(
                            f"  - Mapped dependency '{dep_name}' to ID {task_map[dep_name].id}"
                        )
                    else:
            ...
            for test_task in testing_tasks:
                logger.info(
                    f"  Testing task '{test_task.name}' depends on {len(test_task.dependencies)} task(s)"
                )
            ...
            try:
                ...
                if not self._validate_dependencies(optimized_deps, tasks):
                    ...
                    logger.warning(
                        "Dependency optimization failed validation, using original dependencies"
                    )
                    for task in tasks:
                ...
                logger.warning(
                    "Dependency analysis failed, using original dependencies", error=str(e)
                )
                    ...
                    if task_id in dependencies:
                        ...
                        for dep_id in dependencies[task_id]:
                            try:
                                ...
                                logger.error(
                                    f"Invalid UUID format for dependency: {dep_id}, error: {e}"
                                )
                ...
                if blocked_tasks:
                    ...
                    for task in can_skip:
                        ...
                        await self.task_manager.fail_task(
                            task.id, "Skipped due to failed dependencies"
                        )
                        failed_tasks.add(task.id)

                        # Create a failed result
                        failed_result = TaskResult(task_id=task.id, agent_id=self.id, success=False)
            ...
            for task in ready_tasks:
                try:
                    agent = await self.spawn_agent(
                        task.required_role or AgentRole.CORE_LOGIC, context
                    )
                    ...
                    if status["status"] == "idle" and status["current_task"] is None:
                        ...
                        if last_task_id:
                            ...
                            await self.coordinator.handle_task_completion(
                                agent_id, task_id, True, status.get("execution_time", 0)
                            )
    ...
    async def spawn_agent(self, role: AgentRole, context: Optional[TaskContext] = None) -> Agent:
        ...
        agent = await self.coordinator.spawn_agent(
            ...
            metadata={
                ...
                "project": (
                    self._active_project.get("project_summary", "") if self._active_project else ""
                ),
    ...
    async def _execute_task_with_retry(
        ...
        for attempt in range(self.max_retries):
            ...
            if attempt < self.max_retries - 1:
                ...
                logger.info(f"Retrying task in {retry_delay} seconds", task_id=task_id)
    ...
    async def _calculate_retry_delay(self, task: Task, error: Exception, attempt: int) -> float:
        ...
        return min(self.retry_delay * (self.retry_backoff ** (attempt - 1)), 60.0)  # Max 60 seconds
    ...
    async def _prepare_task_for_retry(self, task: Task, error: Exception, attempt: int) -> None:
        ...
        if diagnosis.retry_strategy == RetryStrategy.MODIFY_PROMPT:
            ...
            new_description = await self.failure_analyzer.generate_fix_prompt(task, diagnosis)
            task.description = new_description
            logger.info("Modified task prompt for retry", task_id=task.id, strategy="modify_prompt")

        elif diagnosis.retry_strategy == RetryStrategy.SIMPLIFY_TASK:
            # Simplify the task
            task.description = await self.failure_analyzer.generate_fix_prompt(task, diagnosis)
            logger.info("Simplified task for retry", task_id=task.id, strategy="simplify_task")
            ...
            if hasattr(task, "timeout"):
                ...
                logger.info(
                    "Increased task timeout for retry", task_id=task.id, new_timeout=task.timeout
                )
...
{{
    ...
    async def aggregate_results(
        self, task_results: dict[UUID, TaskResult]
                        ...
                        failure_reports.append(
                            {
                                ...
                                "final_error": (
                                    task_results[task_id].errors[0]
                                    if task_results[task_id].errors
                                    else "Unknown error"
                                ),
                            }
                        )
                ...
                metadata={
                    ...
                    "task_results": {
                        str(task_id): {
                            ...
                            "artifacts": (
                                len(result.artifacts) if hasattr(result, "artifacts") else 0
                            ),
                            "execution_time": (
                                result.execution_time if hasattr(result, "execution_time") else 0.0
                            ),
                        }
...
Return ONLY the project name, nothing else."""
        ...
        if not existing_projects:
            ...
            for existing in existing_projects:
                if existing == project_name:
                    ...
                    existing.startswith(f"{project_name}_")
                    and existing[len(project_name) + 1 :].isdigit()
                ):
                    try:
    ...
    async def _create_project_manifest(
        self, user_request: str, task_results: dict[UUID, TaskResult]
    ) -> Optional[Artifact]:
        ...
        manifest_data = {
            ...
            "tasks": [
                {
                    ...
                    "success": result.success if hasattr(result, "success") else False,
                    "artifacts": (
                        [str(aid) for aid in result.artifacts]
                        if hasattr(result, "artifacts")
                        else []
                    ),
                    "execution_time": (
            ...
            "statistics": {
                ...
                "total_artifacts": sum(
                    len(r.artifacts) if hasattr(r, "artifacts") else 0
    ...
    async def _export_project(
        self, project_id: str, export_path: Path, include_metadata: bool = True
    ) -> bool:
