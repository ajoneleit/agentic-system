...
class MemoryStorage:
    ...
    def __init__(self, max_size: int = 100):
        ...
        logger.info("MemoryStorage initialized", max_size=max_size)
        ...
        async with self._lock:
            if version is None:
                ...
                if key in self._cache:
                    ...
                    logger.debug("Cache hit", artifact_id=str(artifact_id), version=version)
    ...
    async def delete(self, artifact_id: UUID, version: Optional[int] = None) -> None:
        ...
        async with self._lock:
            if version is None:
                # Delete all versions
                keys_to_delete = [key for key in self._cache if key[0] == artifact_id]
                ...
                if key in self._cache:
                    ...
                    logger.debug(
                        "Artifact removed from cache", artifact_id=str(artifact_id), version=version
                    )
    ...
    async def clear(self) -> None:
        ...
        async with self._lock:
            ...
            logger.info("Cache cleared", artifacts_removed=size_before)
        ...
        async with self._lock:
            ...
            for artifact in self._cache.values():
                ...
                total_metadata_size += (
                    ...
                    # Strings
                    len(artifact.name.encode("utf-8")) + len(artifact.language.encode("utf-8"))
    ...
    async def warmup(self, artifacts: list[Artifact]) -> None:
        ...
        async with self._lock:
            ...
            logger.info("Cache warmed up", artifacts_loaded=len(self._cache))
    ...
    async def has_artifact(self, artifact_id: UUID, version: Optional[int] = None) -> bool:
        ...
        async with self._lock:
            if version is None:
                ...
                return any(aid == artifact_id for aid, _ in self._cache.keys())
            else:
