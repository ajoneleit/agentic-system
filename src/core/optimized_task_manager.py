...
from src.core.interfaces import Task, TaskStatus, TaskPriority
...
class DependencyGraph:
    ...
    def get_ready_tasks(self, limit: int = 10) -> List[Task]:
        ...
        ready_nodes = [
            self.nodes[task_id] for task_id in self.ready_tasks
    ...
    def _is_task_ready(self, task_id: UUID) -> bool:
        ...
        if task_id in self.completed_tasks or task_id in self.failed_tasks or task_id in self.in_progress_tasks:
            return False
    ...
    def _calculate_priority_score(self, task: Task) -> float:
        ...
        complexity_multiplier = {
            ...
            "medium": 1.0,
            "complex": 1.2
        }.get(task.estimated_complexity, 1.0)
    ...
    def _calculate_critical_path(self) -> List[UUID]:
        ...
        while current is not None:
            ...
            if node.dependencies:
                next_task = max(
                    node.dependencies,
                    key=lambda x: longest_paths.get(x, 0)
                )
...
class OptimizedTaskManager:
    ...
    async def get_next_tasks(self, limit: int = None) -> List[Task]:
        ...
        async with self._lock:
            ...
            if self._ready_task_cache and self._cache_expiry and datetime.now() < self._cache_expiry:
            ...
            self._cache_expiry = datetime.now().replace(microsecond=0).replace(second=0)  # Cache for 1 minute
    ...
    async def get_progress(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "blocked_tasks": total_tasks - completed_tasks - failed_tasks - active_tasks - ready_tasks,
    ...
    async def optimize_execution_order(self) -> List[Task]:
        ...
        async with self._lock:
            ...
            for level in sorted(level_groups.keys()):
                ...
                level_tasks.sort(key=lambda t: self._dependency_graph.nodes[t.id].priority_score, reverse=True)
        ...
        async with self._lock:
            ...
            return {
                **self._metrics,
                ...
                "cache_stats": {
                    "ready_cache_size": len(self._ready_task_cache) if self._ready_task_cache else 0,
                    "cache_valid": self._cache_expiry is not None and datetime.now() < self._cache_expiry
                }
            }
