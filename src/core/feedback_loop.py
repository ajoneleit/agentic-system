...
from src.core.interfaces import Agent, AgentRole
...
class FeedbackLoopManager:
    ...
    async def evaluate_output(
        ...
        if quality_result.passed and quality_result.metrics.overall_score >= self.target_quality_score:
    ...
    async def _generate_improvement_prompt(
        ...
        if quality_result.metrics.test_coverage_estimate < 0.8:
            sections.append("- Add comprehensive test cases covering edge cases and error conditions")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_iterations": (
                ...
                if self._feedback_history else 0
            ),
            "average_final_score": (
                sum(h.final_score for h in completed_tasks if h.final_score) / 
                len(completed_tasks)
                if completed_tasks else 0
            ),
            "success_rate": (
                len(completed_tasks) / len(self._feedback_history)
                if self._feedback_history else 0
            )
        }