...
class ArtifactManager:
    ...
    async def _store_artifact_internal(self, artifact: Artifact, force: bool = False) -> Artifact:
        ...
        if not force and await self._version_exists(artifact.id, artifact.version):
            ...
            raise VersionConflictError(artifact.id, artifact.version, existing.version)
        ...
        try:
            ...
            logger.error("Failed to store artifact", artifact_id=str(artifact.id), error=str(e))
    ...
    async def get_artifact(self, artifact_id: UUID, version: Optional[int] = None) -> Artifact:
    ...
    async def get_artifact_versions(self, artifact_id: UUID) -> list[Artifact]:
    ...
    async def get_artifacts_by_task(self, task_id: UUID) -> list[Artifact]:
        ...
        async with self._lock:
            ...
            for aid in artifact_ids:
                try:
                    ...
                    logger.warning(
                        "Artifact missing for task", task_id=str(task_id), artifact_id=str(aid)
                    )

            return artifacts

    async def get_artifacts_by_agent(self, agent_id: UUID) -> list[Artifact]:
        ...
        async with self._lock:
            ...
            for aid in artifact_ids:
                try:
                    ...
                    logger.warning(
                        "Artifact missing for agent", agent_id=str(agent_id), artifact_id=str(aid)
                    )
    ...
    async def delete_artifact(self, artifact_id: UUID, delete_all_versions: bool = False) -> None:
        ...
        async with self._lock:
            ...
            await self._get_artifact_internal(artifact_id)
            ...
            for aid in ids_to_delete:
                try:
                    ...
                    logger.error("Failed to delete artifact", artifact_id=str(aid), error=str(e))
    ...
    async def cleanup_old_versions(self, days: Optional[int] = None, keep_latest: int = 3) -> int:
    ...
    async def export_artifact(
        self, artifact_id: UUID, export_path: Path, include_metadata: bool = True
    ) -> Path:
        ...
        logger.info("Artifact exported", artifact_id=str(artifact_id), export_path=str(export_path))
    ...
    async def _load_metadata(self) -> None:
        ...
        try:
            ...
            logger.info("Metadata loaded", artifacts_count=len(self._artifacts_by_id))
