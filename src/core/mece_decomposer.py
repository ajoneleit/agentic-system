...
from src.core.interfaces import Task, TaskPriority, AgentRole
...
class ContextCompressor:
    ...
    async def compress_partition(
        ...
        compressed = await self._intelligent_compression(
            relevant_context,
            partition.tasks
        )
        ...
        if "artifacts" in global_context:
            ...
            for task in tasks:
                referenced_artifacts.update(
                    task.metadata.get("required_artifacts", [])
                )
            
            relevant["artifacts"] = {
                k: v for k, v in global_context["artifacts"].items()
                if k in referenced_artifacts
            }
...
class MECEDecomposer:
    ...
    async def decompose(
        ...
        final_partitions = []
        for i, partition_tasks in enumerate(optimized):
            partition = TaskPartition(
                id=str(uuid4()),
            ...
            # Compress context
            partition.context = await self.compressor.compress_partition(
                partition,
                global_context
            )
    ...
    def _can_merge(
        self,
        partition1: List[Task],
        partition2: List[Task],
        dep_graph: nx.DiGraph
    ) -> bool:
    ...
    def _validate_partitions(
        ...
        """Validate MECE properties"""
        return (
            self.validator.validate_mutually_exclusive(partitions) and
            self.validator.validate_collectively_exhaustive(partitions, original_tasks) and
            self.validator.validate_dependencies(partitions)
        )
