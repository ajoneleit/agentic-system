...
from src.core.exceptions import AgentCommunicationError
...
class CommunicationHub:
    ...
    async def register_agent(
        ...
        async def message_handler(message: Message) -> Dict[str, Any]:
            ...
            try:
                response = await agent.collaborate(
                    self,  # Pass hub as "other_agent" for context
                    message.to_dict()
                )
        ...
        await self.emit_event("agent_registered", {
            "agent_id": str(agent.id),
            "role": agent.role.value,
            "metadata": metadata,
        })
    ...
    async def unregister_agent(self, agent_id: UUID) -> None:
        ...
        # Emit event
        await self.emit_event("agent_unregistered", {
            "agent_id": str(agent_id),
        })
