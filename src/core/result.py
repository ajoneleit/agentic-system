...
class Result(Generic[T]):
    ...
    def __init__(self, inner: Union[Ok[T], Err[Exception]]):
    ...
    def match(
        self,
        success: Callable[[T], Awaitable[U]],
        failure: Callable[[Exception], Awaitable[U]]
    ) -> Awaitable[U]:
        ...
    
    def match(
        self,
        success: Union[Callable[[T], U], Callable[[T], Awaitable[U]]],
        failure: Union[Callable[[Exception], U], Callable[[Exception], Awaitable[U]]]
    ) -> Union[U, Awaitable[U]]:
...
async def collect_async_results(
    results: List[Awaitable[Result[T]]]
) -> Result[List[T]]:
