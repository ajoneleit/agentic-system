...
def exception_to_result(func: Callable[..., T]) -> Callable[..., Result[T]]:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Result[T]:
        try:
            ...
            logger.debug(f"Converted exception to Result.failure in {func.__name__}", error=str(e))
...
def async_exception_to_result(
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Result[T]:
        try:
            ...
            logger.debug(
                f"Converted exception to Result.failure in async {func.__name__}", error=str(e)
            )
            return Result.failure(e)
...
class AsyncResultChain:
    ...
    def then(self, func: Callable[[T], asyncio.Awaitable[Result[Any]]]) -> "AsyncResultChain":
...
def log_result_errors(component: str, operation: str) -> Callable[[Result[T]], Result[T]]:
...
def validate_result(
    validator: Callable[[T], bool], error_message: str
    ...
    def validate(result: Result[T]) -> Result[T]:
        if result.is_success():
            ...
            if not validator(value):
                return Result.failure(ValueError(f"{error_message}: {value}"))
        return result
...
async def retry_with_result(
    ...
    return Result.failure(last_error or RuntimeError(f"Failed after {max_attempts} attempts"))
...
class CircuitBreaker:
    ...
    def protect(
        self, func: Callable[..., asyncio.Awaitable[Result[T]]]
    ) -> Callable[..., asyncio.Awaitable[Result[T]]]:
