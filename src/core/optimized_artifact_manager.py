...
class OptimizedArtifactManager:
    ...
    async def store_artifacts_batch(self, artifacts: List[Artifact], force: bool = False) -> List[Artifact]:
        ...
        async with self._lock:
            ...
            for i in range(0, len(artifacts), self.batch_size):
                ...
                batch_results = await asyncio.gather(*[
                    self._store_artifact_optimized(artifact, force)
    ...
    async def get_artifact(self, artifact_id: UUID, version: Optional[int] = None) -> Optional[Artifact]:
        ...
        async with self._lock:
            ...
            if cache_misses:
                ...
                filesystem_results = await asyncio.gather(*[
                    self._filesystem_storage.get(aid)
                    for aid in cache_misses
                ], return_exceptions=True)
    ...
    async def _get_artifact_optimized(self, artifact_id: UUID, version: Optional[int] = None) -> Optional[Artifact]:
            ...
            try:
                ...
                if self._write_buffer:
                    ...
                    await asyncio.gather(*[
                        self._filesystem_storage.store(artifact)
                        for artifact in buffer_copy
                    ], return_exceptions=True)
    ...
    async def shutdown(self) -> None:
        ...
        if self._write_buffer:
            await asyncio.gather(*[
                self._filesystem_storage.store(artifact)
                for artifact in self._write_buffer
            ], return_exceptions=True)
