...
class TaskQueue:
    ...
    async def add_task(self, task: Task) -> None:
        ...
        async with self._lock:
            ...
            heapq.heappush(self._heap, (priority_value, task.created_at, task))
    ...
    async def get_next_ready_task(self, completed_tasks: set[UUID]) -> Optional[Task]:
        ...
        async with self._lock:
            ...
            while self._heap:
                ...
                if task.is_ready(completed_tasks):
                    ...
                    logger.debug(
                        f"Task '{task.name}' is ready to execute (all {len(task.dependencies)} dependencies satisfied)"
                    )
                    return task
                else:
                    # Log why task is not ready
                    unsatisfied_deps = [
                        str(d) for d in task.dependencies if d not in completed_tasks
                    ]
                    logger.debug(
    ...
    async def remove_task(self, task_id: UUID) -> Optional[Task]:
        ...
        async with self._lock:
            ...
            self._heap = [(p, c, t) for p, c, t in self._heap if t.id != task_id]
...
class DependencyResolver:
    ...
    async def add_tasks(self, tasks: list[Task]) -> None:
        ...
        async with self._lock:
            ...
            for task in tasks:
                self._graph.add_node(task.id, task=task, name=task.name)
            ...
            if not nx.is_directed_acyclic_graph(self._graph):
                ...
                raise TaskDependencyError(
                    task_id=tasks[0].id, missing_deps=cycles[0] if cycles else []
                )
...
class TaskManager:
    ...
    async def complete_task(self, task_id: UUID, artifacts: Optional[list[UUID]] = None) -> None:
    ...
    async def mark_task_failed(self, task_id: UUID, error: str) -> None:
        ...
        if task:
            ...
            logger.info(
                "Task marked as failed", task_id=str(task_id), task_name=task.name, error=error
            )
